...
- **Storage**: File system only (`~/.aaos-sr-sr/`) ??JSONL logs + Markdown memory
...
- Responsibility: Writes session messages as JSONL to `~/.aaos-sr-sr/` session log file
- Dependencies: Node.js `fs`
...
- Volumes: `~/.aaos-sr-sr/` mounted as persistent volume
...
| `OPENCLAW_WORKSPACE` | Path to workspace dir (default: `~/.aaos-sr`) |
...
| OQ-004 | How is the SkillStore persisted between restarts? | apply-best-practice-default | JSON file at `~/.aaos-sr-sr/skills/registry.json`; loaded at startup |
